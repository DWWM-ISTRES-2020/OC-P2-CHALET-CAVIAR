		...
		<form action="admin.php?page=wpdevart-reservations" method="post" id="reservations_form">
			<div id="action-buttons" class="div-for-clear">
				<div id="reservation_header" class="div-for-clear">
					...
					<?php else : ?>
						<a id="view_calendar" href="<?php echo add_query_arg(array( 'page' => 'wpdevart-reservations', 'task' => 'display_month_reservations' ), admin_url('admin.php')); ?>"><span class="reservation-item-info"><?php _e('Reservation Month View','booking-calendar'); ?></span></a>
					<?php endif; ?>
					<a id="add_reservation" href="<?php echo add_query_arg(array( 'page' => 'wpdevart-reservations', 'task' => 'add' ), admin_url('admin.php')); ?>" class="add-reservation"><span class="plus">+</span><span class="reservation-item-info"><?php _e('Add Reservation','booking-calendar'); ?></span></a>
				</div>
			...
			<table class="wp-list-table widefat fixed pages wpdevart-table wpdevart-reservations-table"> 
				...
				if(isset($_SESSION["clendar_id"]) && $_SESSION["clendar_id"] != 0) {
					foreach ( $rows as $row ) {
						...
						if($row->check_in) {
							$check_in = date_i18n($date_format, strtotime($row->check_in));
							$check_out = date_i18n($date_format, strtotime($row->check_out));
						} else {
							$single_day = date_i18n($date_format, strtotime($row->single_day));
						}
						if(isset($single_day)) {
						...
						<tr>   
							...
							<td colspan="1">
								<div class="created_date"><?php _e('Created:','booking-calendar'); ?> <?php echo date_i18n($date_format. " H:i", strtotime($row->date_created)); ?></div>
							</td>
						</tr>
				<?php	}
				 } ?>
...
<?php
    ...
    public function display_month_reservations() {
		...
		if(isset($_REQUEST["clendar_id"])) {
			$_SESSION["clendar_id"] = $_REQUEST["clendar_id"];
		}
		$calendar_rows = $this->model_obj->get_calendar_rows();  ?>
		...
		<form action="admin.php?page=wpdevart-reservations" method="get" id="reservations_form">
			<div id="action-buttons" class="div-for-clear">
				<div id="reservation_header" class="div-for-clear">
					...
					<select name="clendar_id" onchange="wpdevart_set_value('task','display_month_reservations');this.form.submit()">
						<option value='0'><?php _e('Select Calendar','booking-calendar'); ?></option>
						<?php foreach($calendar_rows as $calendar_row) {
							echo "<option value='".$calendar_row["id"]."' ".selected($_SESSION["clendar_id"], $calendar_row["id"],false).">".$calendar_row["title"]."</option>";
						} ?>
					</select>
					<a id="view_list" href="<?php echo add_query_arg(array( 'page' => 'wpdevart-reservations', 'task' => 'display_reservations' ), admin_url('admin.php')); ?>"><span class="reservation-item-info"><?php _e('Reservation List View','booking-calendar'); ?></span></a>
			...
			<form action="admin.php?page=wpdevart-reservations" method="get" id="reservations_form">
				<div id="action-buttons" class="div-for-clear">
					<div id="reservation_header" class="div-for-clear">
						...
						<select name="clendar_id" onchange="wpdevart_set_value('task','add');this.form.submit()">
							<option value='0'><?php _e('Select Calendar','booking-calendar'); ?></option>
							<?php foreach($calendar_rows as $calendar_row) {
								echo "<option value='".$calendar_row["id"]."' ".selected($_SESSION["clendar_id"], $calendar_row["id"],false).">".$calendar_row["title"]."</option>";
							} ?>
