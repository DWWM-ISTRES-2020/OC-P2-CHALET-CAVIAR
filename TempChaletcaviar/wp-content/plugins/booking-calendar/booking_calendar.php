...
class wpdevart_bc_calendar{
	...
	**/
	...
	public function install_databese(){
		...
		if ($version && version_compare($version, "10.5", '<')) {
			update_option("wpdevart_booking_version", "10.6");
		}
		$version = get_option("wpdevart_booking_version");
	...
	public function wpdevart_time_zone(){
		if (get_option('gmt_offset')) {
			$offset = wpdevart_bc_Library::timezone_from_offset(get_option('gmt_offset')); 
