		...
		if( isset($data["wpdevart-submit".$submit]) ){ 
			...
			if(count($payments)) {
				...
				foreach($payments as $key=>$payment){
					...
					$booking_calendar .= '<button type="button" class="wpdevart-payment-button ' . $button_cl . ' ' . $class . '" id="'.$key.'" name="wpdevart-payment'.$submit.'" data-id="'.$submit.'" data-resid="'.(isset($form_data)? $form_data['reservation_id'] : "").'" data-themeid="'.$this->ids["theme_id"].'">'.$button_content .'</button>';
				}
				...
				$booking_calendar .= '<input type="hidden" name="resid_'.$submit.'" value="'.(isset($form_data)? $form_data['reservation_id'] : "").'">
