...
class wpdevart_bc_Library {
	...
	public static function multi_lng(){
		...
		<div id="multi_lng_popup">
			<div id="multi_lng_popup_container">
				<div><?php echo  __('If you use multilingual Form(Extra), then you need to type translation words instantly from here(not from translation files). Just find the language prefix(code) and add words as it shown in example.','booking-calendar') . '<br>[en]First Name[/en][fr]Prénom[/fr][es]Nombre de pila[/es]'; ?></div>
	...
	*/
	...
	public function send_mail($data, $from, $options, $type = '') {
		...
		if($data['check_in']) {
			$check_in = date_i18n($options["date_format"], strtotime($data['check_in']));
			$check_out = date_i18n($options["date_format"], strtotime($data['check_out']));
			$res_day = $check_in. "-" .$check_out;
		} else {
			$res_day = date_i18n($options["date_format"], strtotime($data['single_day']));
		}
		...
		if($hour_html != ""){
			$hour_html = "<tr><td style='padding: 4px 7px;'>".__('Hour','booking-calendar')."</td> <td  style='padding: 4px 7px;'>".$hour_html.'</td></tr>';
		}
		$res_info = "<table border='1' style='border-collapse:collapse;min-width: 360px;border-color: #c6c6c6;'>
						...
						<tr><td style='padding: 4px 7px;'>".__('Item Count','booking-calendar')."</td><td style='padding: 4px 7px;'>".$data['count_item']."</td></tr>";
		if($data['price'] != "NaN" && !$hide_price){				
			$res_info .= "<tr><td style='padding: 4px 7px;'>".__('Price','booking-calendar')."</td> <td style='padding: 4px 7px;'>".($cur_pos == "before" ? esc_html($currency) : '') . $data['price'] . ($cur_pos == "after" ? esc_html($currency) : '')."</td></tr>";
		}
		if($data['total_price'] != "NaN" && !$hide_price){
			$res_info .= "<tr><td style='padding: 4px 7px;'>".__('Total Price','booking-calendar')."</td> <td style='padding: 4px 7px;'>".$sale_percent_html . ($cur_pos == "before" ? esc_html($currency) : '') . $data['total_price'] . ($cur_pos == "after" ? esc_html($currency) : '')."</td></tr>";
		}
		$res_info .= "</table>";
		$form = "";
		$extras = "";		
		...
		if(count($admin_email_types)) {	
			foreach($admin_email_types as $admin_email_type) {
				...
				if(isset($options[$admin_email_type.'_fromname']) && $options[$admin_email_type.'_fromname'] != "") {
					$fromname = stripslashes($options[$admin_email_type.'_fromname']);
				}
				if(isset($options[$admin_email_type.'_subject']) && $options[$admin_email_type.'_subject'] != "") {
					$subject = stripslashes($options[$admin_email_type.'_subject']);
				}
				if(isset($options[$admin_email_type.'_content']) && $options[$admin_email_type.'_content'] != "") {
					$content = stripslashes($options[$admin_email_type.'_content']);
					...
					$content = str_replace("[ID]", $data["reservation_id"], $content);
		...
		if(count($user_email_types)) {	
			foreach($user_email_types as $user_email_type) {	
				...
				if(isset($options[$user_email_type.'_subject']) && $options[$user_email_type.'_subject'] != "") {
					$subject = stripslashes($options[$user_email_type.'_subject']);
				}
				if(isset($options[$user_email_type.'_fromname']) && $options[$user_email_type.'_fromname'] != "") {
					$fromname = stripslashes($options[$user_email_type.'_fromname']);
				}
				if(isset($options[$user_email_type.'_content']) && $options[$user_email_type.'_content'] != "") {
					$content = stripslashes($options[$user_email_type.'_content']);
					$content = str_replace("[calendartitle]", $this->get_calendar_title($data['calendar_id']), $content);
					$content = str_replace("[details]", $res_info, $content);
					$content = str_replace("[siteurl]", $site_url, $content);
					$content = str_replace("[form]", $form, $content);
