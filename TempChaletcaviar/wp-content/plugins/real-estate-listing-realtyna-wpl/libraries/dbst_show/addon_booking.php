...
if($type == 'booking_cancellation' and !$done_this)
{
	if(trim($value) != '')
	{
		...
		if(!empty($booking_cancellation['term'])) $term_value = ' - <a data-realtyna-lightbox href="#wplbooking_term_'.$field->id.'" style="color:#3073AD;text-decoration: underline;">'.__('Terms', 'wpl').'</a>
...
elseif($type == 'booking_pet' and !$done_this)
{
	if(trim($value) != '')
	{
		...
		$return['type'] = $field->type;
		$return['name'] = __($field->name, 'wpl');
		$return['value'] = isset($booking_pet['name']) ? $booking_pet['name'] : '';
		$return['raw_value'] = $value;
	}
