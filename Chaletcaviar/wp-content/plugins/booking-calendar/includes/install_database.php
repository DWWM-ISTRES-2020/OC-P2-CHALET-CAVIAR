...
class wpdevart_bc_install_database{
	...
	public function update_databese($version){
		...
		if(version_compare("10.3", $version, '>')){
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wpdevart_reservations` ADD `sale_percent` varchar(16) NOT NULL");
		}
		if(version_compare("10.4", $version, '>')){
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wpdevart_forms` ADD `user_id` int(11) NOT NULL");
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wpdevart_extras` ADD `user_id` int(11) NOT NULL");
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wpdevart_themes` ADD `user_id` int(11) NOT NULL");
		}
		if(version_compare("10.6", $version, '>')){
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wpdevart_reservations` ADD `sale_type` varchar(16) NOT NULL");
		}
		if(version_compare("10.7", $version, '>')){
